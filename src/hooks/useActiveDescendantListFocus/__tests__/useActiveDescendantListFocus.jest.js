...
function pressKeyboardForward(count, moveForwardKey) {
...
function generalTestsSuite({ isHorizontal, keepOptionSelected }) {
...
function withDefaultFirstItemFocusSuite({ moveForwardKey, isHorizontal }) {
  it("should keep focus on same item (with same id) after items list change", async () => {
    const onItemClick = jest.fn();
    const { result, rerender } = renderHookForTest({ onItemClick, isHorizontal, keepOptionSelected });
  ...
  it("should visually focus on first item when wrapper is focused", async () => {
    const onItemClick = jest.fn();
    const { result } = renderHookForTest({ onItemClick, isHorizontal, keepOptionSelected });
...
function withoutDefaultFirstItemFocusSuite({ moveForwardKey, isHorizontal }) {
  it("After items list changed should focus on the item with the same index as the item which has been focused before the change", async () => {
    const onItemClick = jest.fn();
    const { result, rerender } = renderHookForTest({ onItemClick, isHorizontal, keepOptionSelected });
